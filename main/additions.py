...
DB_PATH_DEL = r"data\delivery_highest.db"
DB_PATH_BULLISH = r"data\bullish_stocks.db"
CSV_PATH = r"data\stock_d.csv"

def init_db_bullish_stocks():
    conn = sqlite3.connect(DB_PATH_BULLISH)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS bullish_stocks (
            SYMBOL TEXT,
            SIGNAL_DATE TEXT,
    ...
    conn.commit()
    conn.close()
    print("bullish_stocks DB initialized.")

def save_to_bullish_stocks(df):
    conn = sqlite3.connect(DB_PATH_BULLISH)
    df.to_sql("bullish_stocks", conn, if_exists="append", index=False)
    conn.close()
    print(f"Saved {len(df)} records to bullish_stocks.")

def read_db_highest_del():
    conn = sqlite3.connect(DB_PATH_DEL)
    df = pd.read_sql_query("SELECT * FROM delivery_highest", conn)
    conn.close()
    return df
...
def main_function():
    ...
    ['SYMBOL', 'DATE', 'SIGNAL_DATE', 'CURRENT_DELIVERY_PERC',
     ...
     'DELIVERY_SPIKE_RATIO_AVG', 'HIGH_PRICE', 'EMA', 'CLOSE_ABOVE_EMA_HD']
    '''
    ...
    df_final = df_final[['SYMBOL', 'SIGNAL_DATE',
       'CURRENT_DELIVERY_PERC', 'PREV_HIGHEST_DELIVERY_PERC',
       'AVG_DELIVERY_PERC', 'DELIVERY_SPIKE_RATIO_AVG', 'EMA',
       'CLOSE_ABOVE_EMA_HD', 'open', 'high', 'low', 'close', 'volume']]
    ...
    bullish_stocks = bullish_stocks[['SYMBOL', 'SIGNAL_DATE','CURRENT_DELIVERY_PERC','AVG_DELIVERY_PERC', 'DELIVERY_SPIKE_RATIO_AVG']].reset_index(drop=True)
...
def main():
    ...
    conn = sqlite3.connect(DB_PATH_BULLISH)
    last_date_df = pd.read_sql_query("SELECT MAX(SIGNAL_DATE) AS last_date FROM bullish_stocks", conn)
    conn.close()
